...
type Client interface {
	...
	// GetSentinelsInMemory(ip string) ([]int32, error)
	...
	Get(ip, password string, key string) (string, error)
	Set(ip, password string, key string, v string) error
}
...
func (c *client) ResetSentinel(ip string) error {
	...
	_, err := cmd.Result()
	if err != nil {
		return fmt.Errorf("redis ResetSentinel failed: %v", err.Error())
	}
	return nil
}
...
func (c *client) GetSlaveOf(ip, password string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("redis GetSlaveOf failed: %v", err.Error())
	}
...
func (c *client) IsMaster(ip, password string) (bool, error) {
	...
	info, err := rClient.Info("replication").Result()
	if err != nil {
		return false, fmt.Errorf("redis get replication failed: %v", err.Error())
	}
...
func (c *client) Get(ip, password string, key string) (string, error) {
	options := &rediscli.Options{
		Addr:     fmt.Sprintf("%s:%s", ip, redisPort),
		Password: password,
		DB:       0,
	...
	rClient := rediscli.NewClient(options)
	defer rClient.Close()
	return rClient.Get(key).Result()
}

func (c *client) Set(ip, password string, key string, v string) error {
	options := &rediscli.Options{
		Addr:     fmt.Sprintf("%s:%s", ip, redisPort),
		Password: password,
		DB:       0,
	...
	rClient := rediscli.NewClient(options)
	defer rClient.Close()
	return rClient.Set(key, v, 0).Err()
}
